...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_id', type=str, required=False, default='TEMPO', help='model id')
    parser.add_argument('--model', type=str, required=False, default='TEMPO',
    ...
    parser.add_argument('--root_path', type=str,
                        default='/tmp/zfh_1/net_load_forecasting/datasets/OPSD/',
                        help='root path of the data file Correlation_analysis/OPSD')
    parser.add_argument('--data_path', type=str, default='net_load_4.csv', help='data file net_load_1.csv / BE_data_time_gai_1.csv / GE_net_load.csv   /france_net_1.csv Rye_dan_1.csv')
    ...
    parser.add_argument('--enc_in', type=int, default=4, help='encoder input size rye=16 ,france=11,AT,GE=12,BE=13')
   ...
   # hyperparameters
    ...
    parser.add_argument('--gpt_layers', type=int, default=4, help='gpt_layers原来：4 [2, 4, 6, 8]')
    ...
    parser.add_argument('--adapter_dim', type=int, default=64, help='adapter_dim原来：64 [16, 32, 64, 128, 256]')
   # parser.add_argument('--rank', type=int, default=16, help='rank')
    ...
    parser.add_argument('--percent', type=float, default=0.1, help='proportion of in-distribution downstream dataset')
    parser.add_argument('--use_pretrained_gpt2', type=bool, default=True,
                    help='使用预训练GPT-2权重(True)或随机初始化(False)')
    #其他
    ...
    parser.add_argument('--csv_name', type=str,
                        default='GPTNET_1_ture_and_pre_ex_gpt_gpt_GPTNET_1',
                        help='实验名称')
    parser.add_argument('--zhibiao_path', type=str,
                        default='/tmp/zfh_1/net_load_forecasting/PIC/LLM_compsion/GPT4TS/GE/Zhibiao_1.csv',
                        help='指标地址')
    parser.add_argument('--pred_path', type=str,
                        default='/tmp/zfh_1/net_load_forecasting/PIC/LLM_compsion/GPT4TS/GE/GPTNET_1_ture_and_pre_ex_gpt_gpt_GPTNET_1.csv',
                        help='负荷预测值地址')

    # 添加性能指标CSV的路径参数（--cost_path 或 -c）
    parser.add_argument('--cost_path', type=str,
                        default='/tmp/zfh_1/net_load_forecasting/PIC/cost/GE/GPTNET_1_ture_and_pre_ex_gpt_gpt_GPTNET_1.csv',
                        help='参数成本地址')
   ...
   # set gpu id
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            # noise_configs = [
            #     ('clean', 'gaussian', 0.0),
            #     ('gauss_1%', 'gaussian', 0.01),
            #     ('gauss_3%', 'gaussian', 0.03),
            #     ('gauss_5%', 'gaussian', 0.05),
            #     ('gauss_10%', 'gaussian', 0.10),
            #     ('impulse_1%', 'impulse', 0.01),
            #     ('impulse_3%', 'impulse', 0.03),
            #     ('bias_2%', 'bias', 0.02),
            #     ('mixed_5%', 'mixed', 0.05),
            # ]
            #
            # rows = []
            # baseline_means = None  # ★ 存储 clean 基准均值
            #
            # for name, ntype, nratio in noise_configs:
            #     print(f"\n===== 噪声场景: {name} =====")
            #     result = exp.test_with_noise(
            #         setting=setting,
            #         noise_type=ntype,
            #         noise_ratio=nratio,
            #         n_trials=10
            ...
            #     means = result.pop('_means')  # 取出均值，不写入表格
            #
            #     # ★ 第一行（clean）作为基准
            ...
            #
            #     # ★ 退化率计算函数
            #     def calc_deg(now, base, higher_is_better=False):
            #         deg = (now - base) / abs(base) * 100
            #         if higher_is_better:
            #             # R2：负数表示下降（变差）
            #             return f"{deg:+.2f}%"
            #         else:
            #             # MAE/MSE/RMSE/SMAPE：正数表示上升（变差）
            #             return f"{deg:+.2f}%"
            #
            #
            #     result['MAE_deg'] = calc_deg(means['MAE'], baseline_means['MAE'])
            #     result['MSE_deg'] = calc_deg(means['MSE'], baseline_means['MSE'])
            #     result['RMSE_deg'] = calc_deg(means['RMSE'], baseline_means['RMSE'])
            #     result['SMAPE_deg'] = calc_deg(means['SMAPE'], baseline_means['SMAPE'])
            #     result['R2_deg'] = calc_deg(means['R2'], baseline_means['R2'], higher_is_better=True)
            #
            #     rows.append(result)
            #
            # # ★ 整理 DataFrame
            # df = pd.DataFrame(rows)[[
            #     'scenario', 'noise_type', 'noise_ratio',
            #     'MAE', 'MAE_deg',
            #     'MSE', 'MSE_deg',
            #     'RMSE', 'RMSE_deg',
            #     'SMAPE', 'SMAPE_deg',
            ...
            #
            # print("\n" + "=" * 140)
            # print("噪声鲁棒性实验结果（含退化率）")
            # print("=" * 140)
            # print(df.to_string(index=False))
            #
            # df.to_csv(
            #     '/tmp/zfh_1/net_load_forecasting/PIC/noise/BE/TEMPO_noise_robustness_with_degradation.csv',
